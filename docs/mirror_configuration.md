...
When enabled, this saves copies of all JSON project metadata downloaded from [PyPI's JSON API](https://docs.pypi.org/api/json/). These files are used by the <project:#bandersnatch-verify> subcommand.
...
```{note}
The specified server must support [PyPI's JSON API](https://docs.pypi.org/api/json/) for Bandersnatch to mirror any projects.
```
